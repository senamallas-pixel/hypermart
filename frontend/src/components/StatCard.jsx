...
export default function StatCard({ icon: Icon, label, value, sub, accent, onClick }) {
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`bg-white border border-[#1A1A1A]/5 rounded-3xl p-4 sm:p-6 flex flex-col gap-3 sm:gap-4 shadow-sm hover:shadow-xl transition-shadow duration-300${onClick ? ' cursor-pointer' : ''}`}
    >
